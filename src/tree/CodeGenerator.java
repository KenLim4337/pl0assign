...
public class CodeGenerator implements DeclVisitor, StatementTransform<Code>,
    ...
    public Code visitAssignmentNode(StatementNode.AssignmentNode node) {
    	...
    	for (StatementNode.SingleAssignmentNode a : node.getAssignments()) {
    		code.append(a.getExp().genCode(this));
    		code.append(a.getVariable().genCode(this));
    	}
    	
    	for (StatementNode.SingleAssignmentNode a : node.getAssignments()) {
    		code.genStore((Type.ReferenceType)a.getVariable().getType());
    	}
