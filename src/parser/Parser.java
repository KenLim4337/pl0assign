...
import tree.StatementNode.ListNode;
...
public class Parser {
    ...
    //Added KW_SKIP , KW_WHEN and KW_CASE
    ...
    //************ More Sets ***************************/
    private final static TokenSet CASE_LOOP_SET =
            new TokenSet ( Token.KW_DEFAULT, Token.KW_WHEN);
     ...
     */
    //Added SkipStatement & CaseStatement
    ...
    //Multi Assignment Parsing
    private StatementNode.AssignmentNode parseAssignment(TokenSet recoverSet) {

    	StatementNode.AssignmentNode result = new StatementNode.AssignmentNode(tokens.getLocation());
    	
    	if (!tokens.beginRule("Assignment", STATEMENT_START_SET, recoverSet)) {
    		return result;
    	}
    	
    	StatementNode.SingleAssignmentNode a 
    		= parseSingleAssignment(recoverSet.union(Token.BAR));
    	
    	result.addAssignment(a);
    	
    	while (tokens.isMatch(Token.BAR)) {
    		tokens.match(Token.BAR, STATEMENT_START_SET);
    		a = parseSingleAssignment(recoverSet.union(Token.BAR));
    		result.addAssignment(a);
    	}
    ...
    /** Rule: Assignment -> LValue ASSIGN Condition (Single Assign) */
    private StatementNode.SingleAssignmentNode parseSingleAssignment(TokenSet recoverSet) {
        if( !tokens.beginRule( "SingleAssignment", LVALUE_START_SET, recoverSet ) ) {
            return new StatementNode.SingleAssignmentNode( tokens.getLocation(), 
        ...
        tokens.endRule( "SingleAssignment", recoverSet );
        return new StatementNode.SingleAssignmentNode( loc, left, right );
    }
    
    
    //Skip Statement code
    private StatementNode.SkipNode parseSkipStatement( TokenSet recoverSet ) {
    	
    	tokens.beginRule("Skip Statement", STATEMENT_START_SET);
    ...
    private StatementNode.CaseNode parseCaseStatement( TokenSet recoverSet ) {
    	...
    	StatementNode.CaseNode result = new StatementNode.CaseNode ( tokens.getLocation() );
    	
    	if (!tokens.beginRule("Case Statement", STATEMENT_START_SET, recoverSet)) {
    		return result;
    	}
    	
    	tokens.match(Token.KW_CASE); 
    	
    	result.addCondition(parseCondition(recoverSet.union(Token.KW_OF)));
    	
    	tokens.match(Token.KW_OF);
    	
    	while (tokens.isMatch(Token.KW_WHEN)) {
    		 result.addBranch(parseCaseBranch(recoverSet.union(CASE_LOOP_SET)));
    	}
    	
    	if (tokens.isMatch(Token.KW_DEFAULT)) {
    		tokens.match(Token.KW_DEFAULT);
    		result.addDefault(parseStatementList(recoverSet.union(Token.KW_END)));
    	}
    ...
    //Branches
    private StatementNode.BranchNode parseCaseBranch ( TokenSet recoverSet ) {
    
    	tokens.beginRule("Case Branch", Token.KW_WHEN);
    	
    	Location loc = tokens.getLocation();
    	
    	tokens.match(Token.KW_WHEN, CONSTANT_START_SET); //Cannot Fail
    	ConstExp constant = parseConstant(recoverSet.union(Token.COLON));
    	tokens.match(Token.COLON, STATEMENT_START_SET);
    	StatementNode.ListNode list = (ListNode) parseStatementList(recoverSet);
    	
    	tokens.endRule("Case Branch", recoverSet); 
    	return new StatementNode.BranchNode(loc, constant, list);
    ...
    private StatementNode parseReadStatement( TokenSet recoverSet ) {
        ...
        return new StatementNode.SingleAssignmentNode( loc, lval, 
