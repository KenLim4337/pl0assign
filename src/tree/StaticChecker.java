...
import tree.ConstExp.ConstIdNode;
...
public class StaticChecker implements DeclVisitor, StatementVisitor, 
    ...
    //Assignment Node Stuff
    public void visitAssignmentNode(StatementNode.AssignmentNode node) {
    	beginCheck("Assignment");

		List<String> checked = new ArrayList<String>();
		
    	for (StatementNode.SingleAssignmentNode a : node.getAssignments()) {
    		
    		for (String b : checked) {
    			if (a.getVariable().toString().equals(b)) {
    				staticError(a.getVariable().toString() + " assigned more than once", a.getLocation());
    			}
    		}
    		
    		checked.add(a.getVariable().toString());
    		a.accept(this);
    	}
    ...
    /*
    	Bulk changes, check visitAssignmentNode
    */
    ...
    public void visitCaseNode (StatementNode.CaseNode node) {
    	...
    	for(StatementNode s: node.getDefault().getStatements()) {
    		s.accept(this);
    	}
    ...
    public void visitBranchNode (StatementNode.BranchNode node) {
    	...
    	node.getConstant().evaluate();
    	
    	for (StatementNode s : node.getList().getStatements()) {
    		s.accept(this);
    	}
